...
VOID
VmExitHandler (
  PGUEST_REGS GuestRegs//客户机寄存器上下文
)
{
    ULONG64 ExitReason;
    ULONG_PTR GuestEIP;
...
)
{
  ...
  VmxGenerateTrampolineToGuest (GuestRegs, Trampoline);//构造将host的寄存器上下文恢复成客户机上下文的代码

  __vmx_off ();
  clear_in_cr4 (X86_CR4_VMXE);
#pragma warning(disable:4055)
  ((VOID (*)()) &Trampoline) ();//恢复到客户机寄存器上下文，vt卸载完成。
